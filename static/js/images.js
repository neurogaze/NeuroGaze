...
async function predictWebcam() {
  // Set the canvas height and width to be the same as the video.
...
function drawBlendShapes(el, blendShapes) {
    ...
    // Filter out only the blend shapes related to the iris
    const irisBlendShapes = blendShapes[0].categories.filter(shape => {
        return shape.categoryName.includes("eyeLookIn") || shape.categoryName.includes("eyeLookOut");
    });

    let htmlMaker = "";
    // Store the current iris blend shapes
    eyeShapes =  irisBlendShapes;
    irisBlendShapes.forEach(shape => {
        // Store the current shape

//        if(shape.score>0.5){
//            console.log(shape.categoryName, shape.score);
//        }
        // Create the HTML to display the blend shape value
        htmlMaker += `
...
function logIrisLandmarks() {
    for (let i = 0; i < eyeShapes.length; i++) {
        const shape = eyeShapes[i];
        // Check if the user is looking left or right
        if (shape.score > 0.3) {
            console.log(shape);
